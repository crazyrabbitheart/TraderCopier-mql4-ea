...
input int sourceAccountNumber=1199703;  // Source Account Number
int destinationAccountNumber=22081044;  // Destination Account Number
noyes invertTradeCopyDirection=0;  // Invert Trade Copy
input lotSizeTypes lotSizeType=lotSizeType_Multiple; // Lot Size Type
input double lotSizeMultipleOfSource=1.0;  // Lot Size Multiple Of Source
input double lotSizeFixed=0.01; // Fixed Lot Size
double lotSizeRiskPercent=5;  // Lot Size Risk %


tradeCopyModes tradeCopyMode=1;  // Trade Copy Mode
...
void CloseAllOrders()
  {
   ...
   for(int cnt = total-1 ; cnt >= 0; cnt--)
     {
      bool bSelect = OrderSelect(cnt, SELECT_BY_POS, MODE_TRADES);
      if(!bSelect)
         continue;
